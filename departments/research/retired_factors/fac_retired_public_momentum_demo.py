"""Synthetic retired momentum factor example.

This module is intentionally simple and educational. It demonstrates the
factor metadata and contract shape without publishing a live research recipe.
"""
...
from oqp.research.factor_presets import CROSS_SECTIONAL_DAILY_NEXT_OPEN as FACTOR_CONTRACT


FACTOR_ID = "fac_retired_public_momentum_demo"
...
FACTOR_METADATA = {
    ...
    "signal_horizon": "toy_medium_term",
    "execution_style": "close_signal_next_open",
    "execution_mode": "risk_desk",
    "publication_note": "Synthetic demo. Not a production trading signal.",
}


def compute_factor(data: pd.DataFrame, *, lookback: int = 5) -> pd.DataFrame:
    """Return a cross-sectional momentum score from synthetic close prices."""
    ...
    frame["momentum"] = frame.groupby("ticker")["close"].pct_change(lookback)
    frame["factor_score"] = frame.groupby("date")["momentum"].transform(_rank_to_unit_interval)
    frame["factor_score"] = frame["factor_score"].fillna(0.0)

    result = frame.loc[:, ["date", "ticker", "factor_score"]].sort_values(["date", "ticker"])
