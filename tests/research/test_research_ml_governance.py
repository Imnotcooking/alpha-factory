...
class ResearchMLGovernanceTests(unittest.TestCase):
    def test_feature_governance_identifies_quality_redundancy_and_keepers(self) -> None:
        ...
        self.assertEqual(result["metadata"]["features"], 3)
        self.assertNotIn("prob_regime_bull", summary.index)
    ...
    def test_feature_matrix_taxonomy_is_normalized_and_filterable(self) -> None:
        panel = self._feature_panel(days=12, assets=6)
        panel["market_vertical"] = np.where(
            panel["ticker"].str[-1].astype(int) < 3,
            "cn_futures",
            "cn_equities",
        )

        observed = observed_feature_matrix_asset_classes(panel)
        futures = scope_feature_matrix(panel, "FUTURES_CN")
        equities = scope_feature_matrix(panel, "EQUITY_CN")

        self.assertEqual(observed, ["EQUITY_CN", "FUTURES_CN"])
        self.assertEqual(futures["ticker"].nunique(), 3)
        self.assertEqual(equities["ticker"].nunique(), 3)
        self.assertTrue(futures["asset_class"].eq("FUTURES_CN").all())
        self.assertEqual(
            infer_feature_matrix_asset_class(
