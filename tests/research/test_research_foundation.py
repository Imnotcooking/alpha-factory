...
class ResearchFoundationTests(unittest.TestCase):
    ...
    def test_fac_094_emits_only_current_intraday_breakout_scores(self) -> None:
        factor_path = PRIVATE_FACTOR_ROOT / (
        ...
        active = result.loc[result["factor_score"].ne(0.0)]

        self.assertEqual(contract.execution_weight_col, "factor_score")
        self.assertGreater(len(active), 0)
        self.assertNotIn("signal", result.columns)
        self.assertTrue(active["factor_score"].between(0.0, 1.0).all())
        self.assertTrue(
            result.loc[
                ~result["breadth_breakout_long_entry_event"],
                "factor_score",
            ].eq(0.0).all()
        )
        self.assertEqual(result.attrs["factor_id"], module.FACTOR_ID)

    def test_rtr_004_routes_lagged_volatility_quartiles_to_the_next_day(self) -> None:
        router_path = (
            PRIVATE_FACTOR_ROOT.parent
            / "routers"
            / "rtr_004_Intraday_Volatility_Quartile.py"
        )
        if not router_path.exists():
            self.skipTest("private rtr_004 router is not present in this checkout")
        spec = importlib.util.spec_from_file_location("test_private_rtr004", router_path)
