...
class ResearchReproducibilityTests(unittest.TestCase):
    ...
    def test_evidence_ticket_records_analyst_workflow_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            ticket = record_evidence_ticket(
                ...
                metadata={"next_page": "08_Research_Review"},
            )
            record_evidence_ticket(
                str(db_path),
    ...
    def test_evidence_ticket_review_update_preserves_evidence_payload(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            reviewed = update_evidence_ticket_status(
                ...
                metadata_patch={"review_source_page": "08_Research_Review"},
                reviewer="unit_test",
            )
        ...
        self.assertEqual(reviewed["metadata"]["next_page"], "08_Research_Review")
        self.assertEqual(reviewed["metadata"]["review_source_page"], "08_Research_Review")
    ...
    def test_model_artifact_store_and_registry_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            listed = list_model_artifacts(db_path=db_path, factor_id="fac_demo")
            self.assertEqual([row["artifact_id"] for row in listed], ["demo_artifact_v1"])
            self.assertEqual(
                list_model_artifacts(db_path=db_path, model_type="missing"),
                [],
            )
