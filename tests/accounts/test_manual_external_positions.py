...
class ManualExternalPositionsTests(unittest.TestCase):
    def test_atomic_json_writer_validates_and_creates_backup(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "manual_external_holdings.json"
            path.write_text('{"positions": [{"position_id": "old", "symbol": "OLD"}]}', encoding="utf-8")
            payload = {
                "positions": [
                    {
                        "position_id": "new-row",
            ...
            count, backup = write_manual_external_positions_file(payload, path)

            self.assertEqual(count, 1)
            self.assertIsNotNone(backup)
            self.assertTrue(backup.exists())
            self.assertEqual(load_manual_external_positions_file(path)[0]["position_id"], "new-row")
            self.assertEqual(load_manual_external_positions_file(backup)[0]["position_id"], "old")

    def test_json_writer_rejects_duplicate_position_ids(self) -> None:
        payload = {
            "positions": [
                {"position_id": "duplicate", "symbol": "A"},
                {"position_id": "duplicate", "symbol": "B"},
            ]
        }
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(ValueError, "Duplicate position_id"):
                write_manual_external_positions_file(payload, Path(tmp) / "holdings.json")

    def test_inactive_manual_row_can_supply_fx_reference(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            upsert_manual_external_positions(
                ...
                [
                    {
                        "position_id": "inactive-eur-fx-reference",
                        "symbol": "REMOVED",
                        "asset_class": "equity",
                        "quantity": 1,
                        "average_cost": 10,
            ...
            unified = load_latest_account_positions(
                db_path,
                environment="live",
            ...
            converted = unified.loc[unified["symbol"].eq("USDOPT")].iloc[0]
            self.assertAlmostEqual(float(converted["market_value"]), 200 / 1.2)
