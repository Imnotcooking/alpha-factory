...
EMBEDDED = bool(globals().get("_OQP_EMBEDDED_STREAMLIT_PAGE")) or (
    os.environ.get("OQP_EMBEDDED_STREAMLIT_PAGE") == "1"
)

if not EMBEDDED:
    st.set_page_config(page_title="Pattern Scan", layout="wide")
...
def _render_nav(options: dict[str, str], t: dict) -> str:
    ...
    st.markdown(
        """
        <style>
        .st-key-pulse_discovery_nav div[data-testid="stButtonGroup"] {
            width: 100% !important;
            border-bottom: 1px solid rgba(128, 128, 128, 0.28) !important;
        }
        .st-key-pulse_discovery_nav div[role="radiogroup"] {
            width: 100% !important;
            gap: 1.5rem !important;
        }
        .st-key-pulse_discovery_nav button[data-testid^="stBaseButton-pills"] {
            min-height: 2.5rem;
        ...
        .st-key-pulse_discovery_nav button[data-testid="stBaseButton-pillsActive"] {
...
def main():
    ...
    forced_mode = (
        globals().get("_OQP_DISCOVERY_WORKFLOW_MODE")
        or os.environ.get("OQP_DISCOVERY_WORKFLOW_MODE")
        if EMBEDDED
        else None
    )
    if forced_mode in {"discovery", "event_study"}:
        workflow_mode = forced_mode
    else:
        st.title(t["title"])
        st.caption(t["subtitle"])
        workflow_mode = st.segmented_control(
            t["workflow_mode_label"],
            options=["discovery", "event_study"],
            default="discovery",
            format_func=lambda mode: t[f"workflow_mode_{mode}"],
            key="pattern_lab_workflow_mode",
        )
    if workflow_mode == "event_study":
        TickPulseLabPage().render(embedded=True)
        return
    ...
    contract_cols = st.columns([0.6, 0.4])
    ...
    if selected_section == "clock":
        clock_controls = st.columns([0.4, 0.6])
        with clock_controls[0]:
            min_bars = st.slider(
                t["clock_min_bars"],
                5,
                250,
