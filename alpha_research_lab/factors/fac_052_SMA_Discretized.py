...
# ==========================================
FACTOR_ID = "fac_052"
FACTOR_NAME = "Cross-Sectional SMA Discretized 5/20 / 横截面SMA离散信号"
CATEGORY = "Trend / 趋势"
COMPLEXITY = 4
ECONOMIC_RATIONALE = (
    "EN: Isolates the execution impact of 5% block discretization applied to the continuous 5/20 SMA signal. / "
    "ZH: 在连续5/20均线信号上施加5%分块离散化，用于隔离执行层影响。"
)
...
def compute(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lf = lf.with_columns([
        ((pl.col("smoothed_score") / 0.05).round(0) * 0.05).alias("discrete_score")
    ])

    lf = lf.with_columns([
        pl.col("discrete_score").shift(1).over("ticker").fill_null(0.0).alias("factor_score")
    ])
