"""Compact variational-autoencoder feature adapter."""
...
class VAEConfig:
    ...
    random_state: int = 42
    device: str = "cpu"
    training_optimizer: TrainingOptimizerSpec | None = None
...
class VAEFeatureEncoder:
    ...
    def __init__(self, config: VAEConfig | None = None):
        ...
        self.training_optimizer_spec_: TrainingOptimizerSpec | None = None
    ...
    def fit(self, x: pd.DataFrame | np.ndarray) -> "VAEFeatureEncoder":
        ...
        optimizer_spec = self.config.training_optimizer or TrainingOptimizerSpec(
            algorithm="adam",
            learning_rate=self.config.learning_rate,
        )
        optimizer = build_torch_optimizer(self.model.parameters(), optimizer_spec)
        self.training_optimizer_spec_ = optimizer_spec
        self.model.train()
        self.loss_history_ = []
    ...
    def save(self, path: str | Path) -> None:
        ...
        payload = {
            ...
            "training_optimizer_spec": self.training_optimizer_spec_,
        }
    ...
    def load(cls, path: str | Path) -> "VAEFeatureEncoder":
        ...
        encoder.training_optimizer_spec_ = payload.get("training_optimizer_spec")
