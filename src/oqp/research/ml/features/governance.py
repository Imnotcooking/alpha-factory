...
from oqp.data.asset_taxonomy import attach_asset_class
...
LEGACY_MATRIX_ASSET_CLASS = "FUTURES_CN"
LEGACY_MATRIX_NAMES = frozenset(
    {
        "ML_Feature_Matrix.parquet",
        "ML_Stacked_Matrix.parquet",
    }
)
...
def list_matrix_files(base_dir: str | Path) -> list[Path]:
    ...
    for pattern in patterns:
        for path in sorted(runtime_feature_store.rglob(pattern) if runtime_feature_store.exists() else []):
            if path.is_file():
...
def infer_feature_matrix_asset_class(matrix_path: str | Path) -> str:
    """Infer a matrix-level taxonomy lane from its path.

    New matrices should carry an ``asset_class`` or ``market_vertical`` column.
    Path inference exists for single-lane files and the migrated legacy matrix.
    """

    path = Path(matrix_path)
    for part in reversed(path.parts):
        candidate = normalize_market_vertical(part)
        if candidate in ASSET_TAXONOMY:
            return candidate
    if path.name in LEGACY_MATRIX_NAMES:
        return LEGACY_MATRIX_ASSET_CLASS
    return UNKNOWN_ASSET_CLASS


def prepare_feature_matrix_taxonomy(
    df: pd.DataFrame,
    *,
    default_asset_class: str = UNKNOWN_ASSET_CLASS,
) -> pd.DataFrame:
    """Return a matrix with a normalized ``asset_class`` column."""

    default_class = normalize_market_vertical(default_asset_class)
    return attach_asset_class(df, default=default_class)


def observed_feature_matrix_asset_classes(
    df: pd.DataFrame,
    *,
    default_asset_class: str = UNKNOWN_ASSET_CLASS,
) -> list[str]:
    """Return sorted taxonomy lanes represented by a feature matrix."""

    prepared = prepare_feature_matrix_taxonomy(
        df,
    ...
    observed = {
        ...
        for value in prepared["asset_class"].dropna().tolist()
    }
    return sorted(value for value in observed if value)


def scope_feature_matrix(
    df: pd.DataFrame,
    asset_class: str,
    *,
    default_asset_class: str = UNKNOWN_ASSET_CLASS,
) -> pd.DataFrame:
    """Return rows belonging to one normalized taxonomy lane."""

    selected = normalize_market_vertical(asset_class)
    prepared = prepare_feature_matrix_taxonomy(
        df,
        default_asset_class=default_asset_class,
    )
    return prepared.loc[prepared["asset_class"].eq(selected)].reset_index(drop=True)
