"""Legacy macro-HMM training utilities retained for compatibility."""
...
from .hmmlearn_models import MarketHMM
...
def _require_columns(df: pd.DataFrame, columns: tuple[str, ...]) -> None:
    ...
    if missing:
        raise ValueError(
            f"Macro HMM feature matrix missing columns: {', '.join(missing)}"
        )
