...
start_dashboard() {
  ...
  if command -v screen >/dev/null 2>&1; then
    ...
    screen -dmS "$session" /bin/bash -lc \
      "cd '$REPO_ROOT' && ./scripts/platform/start_streamlit_dashboard.sh '$app_path' '$port' '$name' >> '$log_path' 2>&1"
  ...
  nohup /bin/bash -lc \
    "cd '$REPO_ROOT' && ./scripts/platform/start_streamlit_dashboard.sh '$app_path' '$port' '$name'" \
    >> "$log_path" 2>&1 &
