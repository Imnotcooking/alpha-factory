...
for file in "${HEALTH_FILES[@]}"; do
  ...
  if "${SSH_CMD[@]}" "$REMOTE" "test -f '$REMOTE_REPO/runtime/logs/$file'"; then
    remote_health_path="$REMOTE_REPO/runtime/logs/$file"
  elif "${SSH_CMD[@]}" "$REMOTE" "test -f '$REMOTE_REPO/logs/$file'"; then
    remote_health_path="$REMOTE_REPO/logs/$file"
  fi
  if [[ -n "$remote_health_path" ]]; then
    rsync -az --partial -e "${SSH_CMD[*]}" \
      "${REMOTE}:${remote_health_path}" \
...
for profile in live paper; do
  ...
  output="$("${SSH_CMD[@]}" "$REMOTE" \
    "cd '$REMOTE_REPO' && source .venv/bin/activate && if test -f scripts/ops/check_ibkr_server_readiness.py; then readiness_script=scripts/ops/check_ibkr_server_readiness.py; else readiness_script=scripts/check_ibkr_server_readiness.py; fi; PYTHONPATH=src:. python \"\$readiness_script\" --profile '$profile' --adapter-check --json" \
    2>&1)"
  status=$?
  set -e
...
"$PYTHON_BIN" - "$READINESS_DIR" "$REPO_ROOT/runtime/logs/server_ibkr_readiness_health.json" <<'PY'
import json
import sys
